        ...
        */
        ...
        drawItinerary : function(itin) {
            ...
            var markerIcon = L.icon({
                //iconUrl: 'images/018-zero.svg',
                iconUrl: otp.config.resourcePath + 'images/025-circle.svg',
                iconSize: [20, 20], //width, height
                iconAnchor: [8, 8], // //margin left, top
                popupAnchor: [-8, -8],
                className: "path-circle-icon"
            });
            var markerLastIcon = L.icon({
                iconUrl: otp.config.resourcePath + 'images/026-target.svg',
                iconSize: [20, 20], //width, height
                iconAnchor: [8, 8], // //margin left, top
                popupAnchor: [-8, -8],
                className: "path-circle-icon"
            ...
            for(var i=0; i < itin.itinData.legs.length; i++) {
                ...
                if(leg.mode == "WALK") {
                    polyline.setStyle({color: this.getModeColor(leg.mode), weight: 9, opacity: 0.75, dashArray: '2, 15'})
                }else{
                    polyline.setStyle({color: this.getModeColor(leg.mode), weight: 6, opacity: 0.7})
                    var coordinates_polyline = polyline.getLatLngs();
                    var marker_first = L.marker([coordinates_polyline[0].lat, coordinates_polyline[0].lng], {icon: markerIcon});
                    var marker_last = L.marker([coordinates_polyline[coordinates_polyline.length-1].lat, coordinates_polyline[coordinates_polyline.length-1].lng], {icon: markerLastIcon});
                    this.pathLayer.addLayer(marker_first);
                    this.pathLayer.addLayer(marker_last);

                    if(leg.mode == "BUS") {
                        //console.log(leg.tripId)
                        //console.log(leg.tripShortName)
                        //console.log(leg.tripDepartureStop)
                        //#LIVEBUTTON #REALTIME #DACANCELLARE
                        //search tripID into database RFID-Route and get RFID code
                        //if is available search the GPS data by RFID and Date (orario della trip)
                    }
                }
            ...
            if (isMobile) this.webapp.map.lmap.fitBounds(itin.getBoundsArray());
        },

        highlightLeg : function(leg) {
            if(!leg.legGeometry) return;
            var polyline = new L.Polyline(otp.util.Geo.decodePolyline(leg.legGeometry.points));
            polyline.setStyle({ color : "yellow", weight: 16, opacity: 0.5 });
        ...
        drawStartBubble : function(leg, highlight) {
            ...
            if(leg.mode == "BUS" && highlight==false){
                ...
                modeIcon = this.icons.getModeBubble(quadrant, leg.startTime, leg.mode, true, highlight);
            }
